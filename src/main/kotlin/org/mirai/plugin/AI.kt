...
object AI : CompositeCommand(
    ...
    suspend fun MemberCommandSenderOnMessage.main(key: String) {
        val dbObject = SQLiteJDBC(PluginMain.resolveDataPath("AI.db"))
        val entryList =
            dbObject.executeStatement("SELECT * FROM Corpus WHERE fromGroup = ${group.id} AND answer GLOB \"*$key*\" OR question GLOB \"*$key*\" OR keys GLOB \"*$key*\";")
        dbObject.closeDB()
        val r = when {
            entryList.isEmpty() -> "问答包含关键词${key}的条目不存在"
            entryList.size >= 10 -> "问答包含关键词${key}的条目过多(超过十条)，请提供更加详细的关键词"
            else -> {
